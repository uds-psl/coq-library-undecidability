...
Section Reduction.
  ...
  Proof using lp_XY Y_fresh XY_neq.
    ...
    subst. rw /M' in_app_iff. move=> [[|[|]]|].
    - move=> [] *. by congruence.
    - move=> [] *. subst. by rw ?skipn_app ?Nat.sub_diag ?skipn_all.
    - done.
    - have := Y_fresh. rw Forall_forall. by move=> H /H [].
  Qed.
  ...
  Proof using lp_XY Y_fresh XY_neq.
    ...
    { case=> >. have := Y_fresh. rw Forall_forall => H /H /= []. by congruence. }
  ...
  Proof using lp_XY Y_fresh XY_neq.
    ...
    - done.
    - have := Y_fresh. rw Forall_forall => H /H [? ?] ?. eexists.
      constructor; first by apply: rn_refl.
      by constructor; first by (apply: rn_step; [by eassumption | by apply: rn_refl]).
  Qed.
...
Section Reduction.
  ...
  Proof using derivable.
    ...
    elim.
    - move=> ? ?. case=> >. rw /M' in_app_iff. case.
      * case; last done.
  ...
  Proof using non_nil derivable.
    ...
    move=> HM >. rw /M' in_app_iff. case; last by move /HM.
...
Section Reduction.
  ...
  Proof using unique_step_l' unique_step_l derivable XY_neq.
    ...
    move=> v w >. rw /M' in_app_iff. case; first last.
    { move /transition => *.
  ...
  Proof using unique_step_l' unique_step_l derivable XY_neq non_nil.
    ...
    { move: H'ys H'L. rw length_app /=. by nia. }
    move=> ? /rt_rt1n Hxz. rw in_app_iff.
  ...
  Proof using unique_step_l' unique_step_l derivable XY_neq non_nil length_preserving_M.
    move=> >. rw /M' in_app_iff. case; last by move /length_preserving_M.
  ...
  Proof using unique_step_l' unique_step_l derivable XY_neq confluent_M.
    ...
    - case; last done.
      ...
      move /transition => /(_ v2 w2) + H. rw -H.
    ...
    - move /transition => /(_ v1 w1) Hxy2.
      ...
      move=> v2 w2 >. rw /M' in_app_iff -/M'. case.
      { case; last done.
...
Section Reduction.
  ...
  Lemma simulation {x y} : reachable M' x y <-> reachable M x y.
  Proof.
    constructor; apply: reachable_incl=> ?; rw /M /M' ?in_app_iff /=; by firstorder done.
  Qed.

  Lemma confluence : confluent M' <-> confluent M.
  Proof. 
    constructor; apply: confluent_incl; rw /M /M' => ?; rw /= ?in_app_iff /=; by firstorder done.
  Qed.
  ...
  Proof.
    rw /M' /M.
    constructor; apply: length_preserving_incl; move=> ?; rw ?in_app_iff /=; by firstorder done.
  Qed.

  Lemma simulation_step {x y} : step M' x y <-> step M x y.
  Proof.
    constructor; apply: step_incl=> ?; rw /M /M' ?in_app_iff /=; by firstorder done.
  Qed.
...
Proof.
  rw Forall_forall. move=> [[[l1 r1] x1] [[l2 r2] x2]] /fresh_StateP. by lia.
Qed.
...
Proof.
  ...
  { rw /M2 /M1 /AddFreshLoop.M' /DerivableRule.M' ?in_app_iff.
  ...
  { apply: AddFreshLoop.step_fresh_rI; first by (move=> /=; lia).
    apply: rt_step. move: HM => /transition => /(_ [] []). by rw ?app_norm. }
  ...
  { apply: (rt_trans (y := (l, a :: r, z))); apply: rt_step;
      ...
      apply /Reordering.simulation_step; rw /Reordering.M -HM2 /M2.
    - apply: (stepI l r [a] [] [] [a] x z); 
        [done | done | clear; by firstorder done ].
    - apply: (stepI [] [] l (a :: r) l' r' z y); 
        [by rw ?app_norm | by rw ?app_norm | clear; by firstorder done ]. }
  exists (M21 ++ M22). constructor; [| constructor; [| constructor]].
  - suff: weight M2 < weight M + (weight_Instruction (a :: l, r, x, (l', r', y))).
    { rw HM2 weight_split. move: (weight_Instruction _) (weight _). by lia. }
    move: Hop. rw /M2 /DerivableRule.M' /= /weight_Instruction.
    case: (basic (a :: l, r, x, (l', r', y))); first by lia.
    move=> _. case: (basic (l, a :: r, z, (l', r', y))).
    { rw /= ?length_app. by lia. }
    rw /= ?length_app /= ?length_app. by lia.
  - apply /DerivableRule.confluence; first by eassumption.
    apply /Reordering.confluence. rw /Reordering.M -HM2.
    apply /DerivableRule.confluence; first by eassumption.
    apply: AddFreshLoop.confluent_M'; [done | done | by apply: fresh_StateP' | by move=> /=; lia ].
  - apply /DerivableRule.length_preservation; 
      [by eassumption | rw length_app /=; lia | ].
    apply /Reordering.length_preservation. rw /Reordering.M -HM2.
    apply /DerivableRule.length_preservation;
      [by eassumption | rw length_app /=; lia | ].
    apply: AddFreshLoop.length_preserving_M'; [done | done | by apply: fresh_StateP' | by move=> /=; lia ].
  - rw (DerivableRule.boundedness (M21 ++ M22)); first by eassumption.
    rw -(Reordering.boundedness M21 M22).
    rw /Reordering.M -HM2.
    rw -(DerivableRule.boundedness); first by eassumption.
    rw -(AddFreshLoop.boundedness); [done | by apply: fresh_StateP' | by move=> /=; lia | done ].
Qed.
...
Proof.
  ...
  - suff: weight M2 < weight M + (weight_Instruction ([], [a], x, ([], [b], y))).
    { rw HM2 weight_split. move: (weight_Instruction _) (weight _). by lia. }
    rw /weight_Instruction /basic.
    rw /M2 /DerivableRule.M' /= /weight_Instruction /=. by lia.
  - apply /DerivableRule.confluence; first by eassumption.
    apply /Reordering.confluence. rw /Reordering.M -HM2.
    apply /DerivableRule.confluence; first by eassumption.
    apply: AddFreshLoop.confluent_M'; [done | done | by apply: fresh_StateP' | by move=> /=; lia ].
...
Proof.
  ...
  { rw /M3 /M2 /M1 /AddFreshLoop.M' /DerivableRule'.M' ?in_app_iff.
    move: HM. clear. by firstorder done. }
  ...
  { have /fresh_StateP : In ([], a :: r, x, ([], b :: r', y)) M1.
    { rw /M1 /AddFreshLoop.M' ?in_app_iff. by right. }
    unfold z2; lia. }
  have ? : z1 <> z2.
  { have /fresh_StateP : In ([], [a], x, ([a], [], z1)) M1.
    { rw /M1 /AddFreshLoop.M' ?in_app_iff. left. by left. }
    unfold z2; lia. }
  ...
  { apply: (rt_trans (y := ([a], r, z1))); [| apply: (rt_trans (y := ([a], r', z2)))]; apply: rt_step;
      ...
      apply /Reordering.simulation_step; rw /Reordering.M -HM3 /M3.
  ...
  { move=> l'' r'' z. move HZ1: (l'', r'', z1) => Z1 HZ1z. case: HZ1z HZ1.
    move=> >. rw /M2 /AddFreshLoop.M' /M1 /AddFreshLoop.M' ?in_app_iff. case.
  ...
  - suff: weight M3 < weight M + (weight_Instruction ([], (a::r), x, ([], (b::r'), y))).
    { rw HM3 weight_split. move: (weight_Instruction _) (weight _). by lia. }
    move: Hop.
    rw /M3 /DerivableRule.M' /= /weight_Instruction.
    case: (basic ([], a :: r, x, ([], b :: r', y))); first by lia.
    move=> _. case: (basic ([], r, z1, ([], r', z2))).
    { rw /= ?length_app. by lia. }
    rw /= ?length_app /= ?length_app. by lia.
  - apply /DerivableRule.confluence; first by eassumption.
    apply /Reordering.confluence. rw /Reordering.M -HM3.
    ...
    apply: AddFreshLoop.confluent_M'; [ done | done | by apply: fresh_StateP' | by move=> /=; lia ].
  - apply /DerivableRule.length_preservation; [by eassumption | move=> /=; by lia | ].
    apply /Reordering.length_preservation. rw /Reordering.M -HM3.
    ...
    apply: AddFreshLoop.length_preserving_M'; [ done | done | by apply: fresh_StateP' | by move=> /=; lia ].
  - rw (DerivableRule.boundedness (M31 ++ M32)); first by eassumption.
    rw -(Reordering.boundedness M31 M32).
    rw /Reordering.M -HM3.
    rw -(DerivableRule'.boundedness); [ by eassumption | done | done | done | done | ].
    rw -(AddFreshLoop.boundedness); [ done | by apply: fresh_StateP' | by move=> /=; lia | ].
    rw -(AddFreshLoop.boundedness); [ done | by apply: fresh_StateP' | by move=> /=; lia | done ].
Qed.
...
Proof.
  ...
  { (* reduce lhs l size *)
    ...
    move=> M'' [? [? [? HM'']]]. exists M''.
    constructor; [done | constructor; [ done | constructor; [done | by rw -HM'']]]. }
  move: l' => [|a l']; first last.
  { (* reduce rhs l size *)
...
Proof.
  ...
  move: HM'. rw Transform.weight_split => ?.
  have : Transform.weight_Instruction op = 0 by lia. clear.
  rw /Transform.weight_Instruction. 

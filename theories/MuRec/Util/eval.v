...
From Undecidability.Shared Require Import DLW.Utils.finite DLW.Vec.vec DLW.Vec.pos PSL.Tactics.Tactics Dec.
From Undecidability.MuRec Require Import MuRec recalg ra_sem_eq.

From Stdlib Require Import List Lia PeanoNat Arith.Wf_nat.
...
#[local] Notation " [ f ; v ] ▹ x " := (@ra_bs _ f v x).
...
Fixpoint erase k (f : recalg k) : reccode :=
  ...
  | ra_zero => rc_zero
  | ra_succ => rc_succ
  | ra_proj p => rc_proj (pos2nat p)
  | ra_comp f g => rc_comp (erase f) (rec_erase erase g)
  | ra_rec f g => rc_rec (erase f) (erase g)
  | ra_min f => rc_min (erase f)
  end.

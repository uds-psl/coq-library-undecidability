...
Proof.
  ...
  - move=> > /pure_typingE [n] [?] [?] [+] []. case: t; last done.
    all: (case: n; [rw map_ren_poly_type_id /=; subst | done]).
...
Proof.
  elim.
  - move=> > [] > /=. apply: rt_step. rw poly_type_norm /=.
    have := contains_step_subst.
    evar (s' : poly_type) => /(_ s'). evar (t' : poly_type) => /(_ t').
    congr contains_step. subst t'. rw poly_type_norm /=.
...
Proof.
  ...
  - move=> > [] > /=. apply: rt_step. rw poly_type_norm /=.
    ...
    move=> ?. by rw /= poly_type_norm /= subst_poly_type_poly_var.
  - move=> ?. by apply: rt_refl.
  - move=> *. apply: rt_trans; by eassumption.
Qed.
...
Proof.
  ...
  rw poly_type_norm /= ren_poly_type_id'; by [|lia].
Qed.
...
Proof.
  ...
  - move=> > [] r > _ IH [|n] > /=; first done.
    ...
    rw subst_poly_type_many_poly_abs /= => /IH [?] [?] [?] [->].
...
Proof.
  ...
  rw nth_error_map Hx /=. by rw ren_poly_type_id.
Qed.
...
Proof.
  move=> HM. apply: (pure_typing_pure_abs 0). by rw map_ren_poly_type_id.
Qed.
...
Proof.
  ...
  { move=> *. rw ?map_map. apply: map_ext => ?.
    rw poly_type_norm /=. apply: extRen_poly_type. by lia. }
  case: M.
  - move=> x /pure_typingE [n'] [?] [?] [+] [HC] ->.
    rw Hnn' many_poly_abs_many_poly_abs => ?. 
...
Proof.
  ...
  { move=> >. rw ?poly_type_norm /σ' /=. apply: ext_poly_type.
    move=> ?. by rw iter_up_poly_type_poly_type. }
  move=> H. elim: H σ.
  - move=> n {}Gamma x > Hx + σ => /(contains_subst_poly_typeI (σ' n σ)) HC.
    rw subst_poly_type_many_poly_abs. apply: (pure_typing_pure_var n); last by eassumption.
    move: Hx. rw ?nth_error_map. case: (nth_error Gamma x); last done.
    move=> ? [<-] /=. by rw Hσ'.
  - move=> n > _ + _ + + σ => /(_ (σ' n σ)) + /(_ (σ' n σ)) + /(contains_subst_poly_typeI (σ' n σ)) ?.
    rw ?map_map. under map_ext => ? do rw Hσ'. move=> ? ?. 
    rw subst_poly_type_many_poly_abs. apply: (pure_typing_pure_app n); rw ?map_map; by eassumption.
  - move=> n > _ + σ => /(_ (σ' n σ)). rw /= map_map. under map_ext => ? do rw Hσ'. move=> ?.
    rw subst_poly_type_many_poly_abs /=. apply: (pure_typing_pure_abs n).
    rw ?map_map. by eassumption.
Qed.
...
Proof.
  ...
  { move=> n >. rw ?poly_type_norm [RHS]ren_as_subst_poly_type /σ.
    apply: ext_poly_type => y /=. rw iter_up_poly_type_poly_type_ge; first by lia.
  ...
  - move=> ? IH1 ? IH2 s Gamma t /pure_typingE [[|n]] [?] [?] [?] [+] [+] [].
    ...
    + move=> /(pure_typing_subst_poly_type (σ n s)) /= + /(pure_typing_subst_poly_type (σ n s)) /= +.
      rw ?map_map. under map_ext => ? do rw Hσ. move=> ? ?.
      move=> /(contains_subst_poly_typeI (σ n s)) ? [->]. rw subst_poly_type_many_poly_abs.
      apply: (pure_typing_pure_app n); by eassumption.
  - move=> ? IH s Gamma t /pure_typingE [[|n]] [?] [?] []; first done.
    move=> + [->]. rw subst_poly_type_many_poly_abs.
...
Proof.
  ...
  - move=> > /typing_var /typing_contains H /H {H} [?] /= [->] /typing_many_ty_absI ?.
    eexists. constructor; last by eassumption. by rw erase_many_ty_abs.
...
Proof.
  ...
  rw allfv_poly_type_ren_poly_type IH.
...
Proof.
  ...
  { subst b2. apply /(sameP fresh_inP) /(equivP fresh_inP).
    rw /= allfv_poly_type_ren_poly_type. apply: ext_allfv_poly_type. by case. }
  case: b2 Hb2; last by rw /= IH.
  rw IH ?poly_type_norm => /fresh_inP H. 
  apply: ext_ren_poly_type_allfv_poly_type. rw allfv_poly_type_tidy.
...
Proof.
  ...
  have ->: b1 = b2.
  { subst b2. apply /(sameP fresh_inP) /(equivP fresh_inP).
    rw /= allfv_poly_type_subst_poly_type.
    apply: ext_allfv_poly_type. case; first done. 
    move=> ?. rw /= allfv_poly_type_ren_poly_type /=. constructor; last done.
    move=> _. by apply: allfv_poly_type_TrueI. }
  case: b2 Hb2.
  - rw IH ?poly_type_norm => /fresh_inP H.
    apply: ext_subst_poly_type_allfv_poly_type. rw allfv_poly_type_tidy.
    apply: allfv_poly_type_impl H. case; first done.
    move=> ? _ /=. by rw tidy_ren_poly_type ?poly_type_norm ren_poly_type_id.
  - move=> _ /=. rw IH /=. congr poly_abs. apply: ext_poly_type. case; first done.
    move=> ?. by rw /= tidy_ren_poly_type.
Qed.
...
Proof.
  ...
  - move: IH. rw tidy_subst_poly_type. congr contains.
    rw ren_as_subst_poly_type. apply: ext_subst_poly_type_allfv_poly_type.
    rw allfv_poly_type_tidy. move: H => /fresh_inP.
    apply: allfv_poly_type_impl. by case.
  - apply: rt_trans; last by eassumption.
    apply: rt_step. rw tidy_subst_poly_type.
...
Proof.
  ...
  - move=> x Gamma t /typingE Hx. exists (var x). constructor; first done.
    apply: typing_var. by rw nth_error_map Hx.
  ...
  - move=> P IH Gamma t /typingE [t'] /= [->] /IH [P'] [->] /=.
    ...
    + move=> /(typing_ren_poly_type (0 .: id)) => H.
      exists (ren_term (0 .: id) id P'). constructor; first by rw erase_ren_term_id.
      congr typing: H. rw ?map_map. apply: map_ext => ?.
      by rw tidy_ren_poly_type ?poly_type_norm /= ren_poly_type_id.
    + move=> H. exists (ty_abs P'). constructor; first done.
      apply: typing_ty_abs. congr typing: H. rw ?map_map.
...
Proof.
  elim: n t.
  - move=> t _ /=. rw ren_poly_type_id'; by [|lia].
  - move=> n IH t /= Ht.
    have ->: fresh_inb 0 (many_poly_abs n t) = true.
    { apply /fresh_inP. rw /fresh_in allfv_poly_type_many_poly_abs.
      apply: allfv_poly_type_impl Ht. move=> x ?. rw iter_scons_ge; by lia. }
    rw IH.
    { apply: allfv_poly_type_impl Ht. by lia. }
    rw poly_type_norm. apply: extRen_poly_type.
...
Proof.
  elim: n s.
  - move=> s. exists (0, id). by rw ren_poly_type_id.
  - move=> n IH s /=. case Hns: (fresh_inb 0 (many_poly_abs n s)).
    + have := IH s. move=> [[n' ξ']] ->.
      rw ren_poly_type_many_poly_abs -tidy_ren_poly_type ?poly_type_norm.
...
Proof.
  ...
  - exists [] => /=. constructor; first by lia.
    by rw (ltac:(lia) : n - 0 = n) subst_poly_type_poly_var. 
  - move=> > [] r ? _ [ts] [?].
    move Hn': (n - length ts) => n'. case: n' Hn'; first done.
    move=> n' ? [->]. exists (r :: ts) => /=.
    constructor; first by lia.
    have ->: n - S (length ts) = n' by lia.
    rw ?poly_type_norm. apply: ext_poly_type. case; first done.
    move=> x /=. by rw ?poly_type_norm /= subst_poly_type_poly_var.
Qed.
...
Proof.
  ...
  exists ts. constructor; [by lia | by rw H Hnts].
Qed.
...
Proof.
  elim: ts t.
  - move=> /= ?. rw subst_poly_type_poly_var. by apply: rt_refl.
  - move=> r ts IH t. rw [length _]/=.
    have ->: S (length ts) = (length ts) + 1 by lia. 
    rw -many_poly_abs_many_poly_abs /=.
    ...
    rw ?poly_type_norm /=. apply: ext_poly_type.
...
Proof.
  ...
  have -> : n = length ts by rw /ts length_map length_seq.
  ...
  have ->: subst_poly_type (fold_right scons poly_var ts) s = subst_poly_type σ s.
  {
    ...
    rw /ts fold_right_length_ts_lt; first by (rw length_map length_seq; lia).
    rw [nth _ _ (poly_var 0)](nth_indep _ _ (σ 0)); first by (rw length_map length_seq; lia).
...
Proof.
  move=> Hs Ht. rw -tidy_many_poly_abs_tidy -/(map tidy [s]). 
...
Proof. elim t; [done | by move=> ? IH1 ? IH2 /=; rw IH1 IH2 | done]. Qed.
...
Proof.
  ...
  rw tidy_is_simple; first by rw is_simple_ren_poly_type.
  rw ren_poly_type_many_poly_abs.
  have -> : ren_poly_type (Nat.iter n up_ren (Nat.add n')) s = s.
  {
    rw -[RHS]ren_poly_type_id.
    apply: ext_ren_poly_type_allfv_poly_type. apply: allfv_poly_type_impl Hns.
    move=> ? ?. rw iter_up_ren_lt; by [|lia].
  }
  rw ren_as_subst_poly_type. by apply: contains_many_poly_abs_closedI.
Qed.
...
Proof.
  ...
  rw -tidy_ren_poly_type ren_poly_type_many_poly_abs.
  have -> : ren_poly_type (Nat.iter n up_ren (Nat.add n)) s = s.
  {
    rw -[RHS]ren_poly_type_id.
    apply: ext_ren_poly_type_allfv_poly_type. apply: allfv_poly_type_impl Hns.
  ...
  rw -[s in contains _ s](tidy_is_simple Hs).
  apply: contains_tidyI.
  rw -[s in contains _ s]subst_poly_type_poly_var.
...
Proof.
  ...
  - move=> n > /(pure_typing_ren_poly_type (fun x => x - n)) /=.
    rw ?map_map. rw (map_ext _ id) ?map_id.
    { move=> ?. rw poly_type_norm ren_poly_type_id' /=; by [|lia]. }
...
Proof.
  ...
  - move=> [?] /pure_typing_weaken_closed => /(_ (many_poly_abs n s)).
    ...
    rw -/([_] ++ map tidy Gamma2) -/([_] ++ Gamma2) ?app_assoc.
    by move=> /pure_typableI /IH.
  - move=> [?] /pure_typing_weaken_closed => /(_ (tidy (many_poly_abs n s))).
    apply: unnest.
    {
      rw allfv_poly_type_tidy allfv_poly_type_many_poly_abs. apply: allfv_poly_type_impl Hns.
      move=> *. rw iter_scons_lt; by [|lia].
    }
...
Proof.
  ...
  - elim; first done.
    ...
    apply: unnest; first by case. rw ren_pure_term_id.
    move=> /(pure_typing_pure_abs 0 (Gamma := [])) /pure_typableI /IH.
    rw /many_pure_term_abs (ltac:(lia) : S n = n + 1) -iter_plus. apply.
    move=> /=. apply: allfv_pure_term_impl HnM.
    case; first done. move=> /=. by lia.
  - move=> s Gamma IH [|n] M.
    + move=> HM [tM].
      move=> /(pure_typing_ren_pure_term_allfv_pure_term id (Delta := [])).
      rw ren_pure_term_id. apply: unnest; first by apply: allfv_pure_term_impl HM; lia.
      by move=> /pure_typableI.
    + move=> HnM [tM]. rw -[Gamma]map_ren_poly_type_id.

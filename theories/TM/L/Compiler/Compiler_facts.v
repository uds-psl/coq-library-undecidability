...
to_list (Vector.rev_append_tail v w) = (rev (to_list v) ++ to_list w)%list.
Proof.
unfold to_list. revert m w. depind v;cbn. reflexivity.
intros. specialize IHv with (w:=h::w). rewrite IHv.
autorewrite with list. easy.
Qed.

Lemma vector_rev_to_list A (n : nat) (v : Vector.t A n): 
  to_list (Vector.rev v) = rev (to_list v).
Proof.
  unfold Vector.rev,Vector.rev_append.
  specialize (vector_rev_append_tail_to_list v []) as H'. cbn in H'.
  autorewrite with list in H'. rewrite <- H'. generalize (rev_append_tail v []). 
  generalize (plus_tail_plus n 0). generalize (tail_plus n 0). generalize (plus_n_O n).
...
Proof.
  ...
  setoid_rewrite <- rev_involutive at 2. rewrite fold_left_rev_right. f_equal.
  rewrite vector_rev_to_list,rev_involutive. easy.
Qed.

...
Proof.
  destruct n; cbn.
  - eexists. eauto with tm.
  - eexists. eapply Switch_Realise. { eauto with tm. }
    cbn in *. intros [ | ].
    instantiate (1 := fun o => match o with None => _ | Some _ => _ end).
    eapply Seq_Realise. { eauto with tm. }
    eapply Switch_Realise. { eauto with tm. } cbn in *. intros [[] | ].
    instantiate (1 := fun o => match o with None => _ | Some true => _ | Some false => _ end). cbn in *.
    now eauto with tm. eapply Switch_Realise. eapply (proj2_sig (ReadB_canonical n)).
    instantiate (1 := fun o => match o with None => _ | Some _ => _ end).
    intros []; now eauto with tm. { eauto with tm. } cbn. eauto with tm.
Defined. (* because definition *)
...
(*
Lemma isTotal_Realise_RealiseIn {Σ} {L} {n} {M : pTM Σ L n} : 
  isTotal m -> M ⊨ R -> pM ⊨c(k) R
*)
...
Proof. 
  induction n. 
  - cbn. eexists. eapply TerminatesIn_monotone. now eauto with tm.
    intros ? ? H. exact H.
  - destruct IHn as [c IH].
    eexists. eapply TerminatesIn_monotone. cbn.
    eapply Switch_TerminatesIn. now eauto with tm. now eauto with tm.
    intros []. instantiate (1 := fun f => if f then _ else _). cbn.
    instantiate (1 := ltac:(clear f; refine _)).
    eapply Seq_TerminatesIn. now eauto with tm. now eauto with tm.
    eapply Switch_TerminatesIn. eauto with tm. eauto with tm.
    intros []. instantiate (1 := fun f => match f with Some true => _ | Some false => _ | None => _ end). cbn.
    destruct b0 as [[] | ].
    + cbn. instantiate (1 := ltac:(clear f b0; refine _)). eauto with tm.
    + instantiate (1 := ltac:(clear f b0; refine _)).
      ...
      intros []; eauto with tm.
    + cbn. instantiate (1 := ltac:(clear f; refine _)). eauto with tm.
    + cbn. instantiate (1 := ltac:(clear f; refine _)). eauto with tm.
...
Proof.
  ...
  { apply Switch_Realise; [now eauto with tm|].
    instantiate (1 := fun x => match x with Some _ => _ | _ => ltac:(clear x; refine _) end).
    intros [?|]; cbn. { now eauto with tm. }
    apply Seq_Realise; [now eauto with tm|].
    apply Switch_Realise; [now eauto with tm|].
    instantiate (1 := fun x => match x with Some _ => _ | _ => _ end).
    intros [?|]; now eauto with tm. }
...
Proof.
  ...
  apply Switch_TerminatesIn; [now eauto with tm..|].
  { instantiate (1 := fun x => match x with Some _ => _ | _ => ltac:(clear x; refine _) end).
    intros [?|]; cbn.
    { now eauto with tm. }
    apply Seq_TerminatesIn; [now eauto with tm..|].
    apply Switch_TerminatesIn; [now eauto with tm..|].
    instantiate (1 := fun x => match x with Some _ => _ | _ => _ end).
    intros [?|]; now eauto with tm. }
...
#[export] Hint Extern 0 (TestLeftof ⊨ _) => apply TestLeftof_Realise : tm.
...
Proof.
  ...
  - eapply Realise_monotone. now eauto with tm. intros t ([], t') ->.
    destruct_tapes. reflexivity.
  - eapply Realise_monotone. eapply Seq_Realise. eassumption. now eauto with tm.
    intros t ([], t') ?; cbn in *. TMSimp. destruct_tapes. TMSimp. reflexivity.
Qed.

#[local] Hint Extern 0 (MoveM _ _ ⊨ _) => apply MoveM_Realise : tm.
...
Proof.
  ...
  - eapply Realise_monotone. unfold WriteB. eapply Switch_Realise.
    ...
    + cbn in *. TMSimp. rewrite <- app_assoc. cbn -[skipn]. repeat f_equal.
      ...
      * now rewrite <- app_assoc.
  - cbn. eapply Realise_monotone. now eauto with tm.
    intros t ([], t') ->. eauto.
Qed.
...
Proof.
  ...
  - instantiate (1 := ltac:(destruct c; refine _ )). cbn.
    apply Switch_TerminatesIn; [eauto with tm..|].
    instantiate (1 := fun x => if x then _ else _).
    intros []; now eauto with tm.
  - cbn. now eauto with tm.
  - cbn. intros ? ? ?. destruct c; cbn.
    + repeat eexists. eapply le_plus_l.
      unshelve eapply (le_trans _ _ _ _ H).
      eapply le_plus_l.
      intros. TMSimp. destruct_tapes. cbn.
      destruct yout.
      * cbn. repeat (rewrite !app_length, ?rev_length, ?length_encode_sym; cbn). eapply le_plus_l.
      * repeat eexists. eapply le_plus_l.
        repeat (rewrite !app_length, ?rev_length, ?length_encode_sym; cbn).
        eapply le_plus_r.
        eapply le_plus_l. eapply le_plus_l.
        intros. TMSimp. eapply le_plus_l.
    + lia.
    Unshelve. all: exact 0.
Qed.
...
Proof.
  ...
  - cbn. intros ? ? ?. repeat eexists.
    ...
    + destruct m; cbn.
      * repeat eexists. all: intros; eapply le_plus_l.
      * eapply le_plus_r.
      * repeat eexists. all: intros; eapply le_plus_l.
    + repeat eexists. all: intros; eapply le_plus_l.
  Unshelve.
...
Section FixM.
  ...
  #[local] Hint Extern 0 (ReadB _ ⊨ _) => apply ReadB_Realise' : tm.
  #[local] Hint Extern 0 (WriteB _ ⊨ _) => apply WriteB_Realise' : tm.
  #[local] Hint Extern 0 (MoveB _ _ ⊨ _) => apply MoveB_Realise' : tm.
  ...
  Proof.
    ...
    {
      unfold Step. eapply Switch_Realise. now eauto with tm. cbn.
  ...
  #[local] Hint Extern 0 (Step _ ⊨ _) => apply Step_Realise : tm.
  ...
  Proof.
    ...
    - unfold Step. eapply Switch_TerminatesIn.
      now eauto with tm. cbn in *. eapply H0. cbn.

...
Record SBTM2 := {
  num_states : nat;
  trans : Fin.t (S num_states) * bool -> option (Fin.t (S num_states) * bool * direction) }.
...
Definition SBTM2_HALT : (SBTM2 * tape) -> Prop :=
  fun '(M, t) => exists k, steps M k (Fin.F1, t) = None.

(* SBTM2 halting problem on the empty tape "([], false, [])" *)
Definition SBTM2_HALT0 : SBTM2 -> Prop :=
  fun M => exists k, steps M k (Fin.F1, (nil, false, nil)) = None.

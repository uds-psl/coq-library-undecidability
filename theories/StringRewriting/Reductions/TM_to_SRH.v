...
Proof.
  exists (fun '(P, x, A) => (P ++ map (fun a => [a] / [fresh (Undecidability.StringRewriting.Util.Definitions.sym P ++ x)]) A, x, fresh (Undecidability.StringRewriting.Util.Definitions.sym P ++ x))).
  intros [ [R x] A]. cbn. firstorder.
  - eapply in_split in H1 as (? & ? & ->). exists (x2 ++ [fresh (Undecidability.StringRewriting.Util.Definitions.sym R ++ x)] ++ x3). split.

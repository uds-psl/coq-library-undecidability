...
Proof using confluent_M.
  ...
  case; [|case].
  - move /(f_equal unembed). rw ? embedP. by move=> ->.
...
Lemma SM_to_SUcsP {x y: state} {a b: symbol} : In (a, x, (y, b)) (SM_to_SUcs M) -> exists x' y', 
  ...
  equiv ([a], [], x') ([], [b], y').
Proof.
  rw /SM_to_SUcs in_map_iff. move=> [[[[[x' y'] a'] b'] d]]. case: d.
  - case. case=> <- <- <- <- H.
...
Proof using confluent_M.
  ...
  - move=> A B A' B' HnB Hxx'.
    ...
    { rw ? ζ_0P. do 2 f_equal. by apply: nf_equiv_eq. }
    rw HnB' ζ_0P ζ_SnP.
  ...
  - move=> m IH A B A' B' Hm Hxx'.
    ...
    + move=> _ _. apply: (arr_eqI (s := fun=> ζ _ _) (t := fun=> ζ _ _)).
      move=> b. have -> : n - length B' = S n - length (B' ++ [b]) by (rw length_app /length; lia).
      apply: IH; [ rw length_app /length -/(length _); by lia | by apply: equiv_appR].
...
Proof using confluent_M.
  ...
  elim: m x A B Hm.
  - move=> x A B Hm. rw ? ζ_0P /= ψP.
    move HAxB: (nf (A, B, x)) => [[A' B'] x'].
    have [-> | ?]: S n - length B' = 0 \/ S n - length B' > 0 by lia.
    { rw ζ_0P. f_equal. f_equal. 
      apply: nf_equiv_eq. apply: equiv_appL. rw -HAxB. by apply: nf_equiv. }
    have Hxx': equiv (A' ++ [a], B', x') (A ++ [a], B, x).
    { apply: equiv_appL. rw equiv_sym -HAxB. by apply: nf_equiv. }
    by rw (ζ_equivP Hn Hxx') Hm ζ_0P.
  - move=> m IH x A B Hm.
    rw ? ζ_SnP. case: (narrow_dec (A, B, x)).
    (* AxB is narrow *)
    + move=> /= /(narrow_appL (a := a)) => ?. case: (narrow_dec (A ++ [a], B, x)); last done.
      move=> _ /=. apply: (arr_eqI (s := fun => ζ _ _) (t := fun => substitute _ _)).
      move=> b. apply: IH. rw length_app /length -/(length _). by lia.
    + move=> _ /=. rw -(ζ_SnP (A := A ++ [a])).
      rw ψP. move HAxB: (nf (A, B, x)) => [[A' B'] x'].
      rw -Hm.
      have Hxx': equiv (A ++ [a], B, x) (A' ++ [a], B', x').
      { apply: equiv_appL. rw -HAxB. by apply: nf_equiv. }
      by rw (ζ_equivP Hn Hxx').
Qed.
...
Proof.
  ...
  - move: HM. rw /SM_to_SUcs Forall_map Forall_forall => HM. 
    move /HM. rw /models /interpret.
    case: (φ (embed ([], [], y))); first done.
    move: a b => [|] [|] ? ? -> /=; by rw ascend_arr.
  - move: HM. rw /SM_to_SUcs Forall_map Forall_forall => HM. 
    move /HM. rw /models /interpret.
...
Proof.
  case: M=> M confluent_M /=.
  move=> [φ [ψ0 [ψ1]]]. rw -Forall_forall => Hφ.

...
Section FailureMonad.
  ...
  *)
  ...
  Notation "x <- c1 ;; c2" := (bind c1 (fun x => c2)) (at level 50).
...
Notation "x <- c1 ;; c2" := (bind c1 (fun x => c2)) (at level 50, c1 at level 51, c2 at level 50).
...
Section TarskiMerging.
  ...
  Definition mFalse := (@falsity fs ps full_operators).
  ...
  * intros [pP pQ]. split. now apply pPr. now apply pQr.
  ...
  Definition mAnd {ff : falsity_flag} := (@bin fs ps full_operators ff Conj).
  ...
  * intros [pP|pQ]. left; now apply pPr. right; now apply pQr.
  ...
  Definition mOr {ff : falsity_flag} := (@bin fs ps full_operators ff Disj).
  ...
  * intros [q Pq]. exists q. destruct (pR q) as [pRl pRr]. now apply pRr.
  ...
  Definition mExists {ff : falsity_flag} := (@quant fs ps full_operators ff Ex).
  ...
  * cbn. intros pPQ pP. apply pQr, pPQ, pPl, pP.
  ...
  Definition mImpl {ff : falsity_flag} := (@bin fs ps full_operators ff Impl).
  ...
  Definition mTrue := (@bin fs ps full_operators falsity_on Impl mFalse mFalse).
                            ...
                            := match s with "and" => reifyAndNP | "or" => reifyOrNP | "ex" => reifyExistNP |  "False" => reifyFalseNP |  "True" => reifyTrueNP | 
                                       _ => match baseLogicConnHelper with 
                                             ...
                                             Some k => fun tct qff lst fuel envTerm env fPR fTR => both <- k s tct qff lst fuel envTerm env 
                                                                                                        ...
                                                                                                        (fun tct qff t eT env => rr <- fTR qff tct t eT env;; ret (rr, noProofDummy));; 
  ...
  Fixpoint findTermRepresentationNP (tct:Ast.term) (fuel:nat) (qff:Ast.term) (t:Ast.term) (termEnv:Ast.term) (env:Ast.term -> FailureMonad nat) {struct fuel}: (FailureMonad (Ast.term)) := match fuel with 
   ...
   (** *** Forms *)
    ...
    | (0,(tApp arg lst)) => if Checker.eq_term init_graph arg qI_P then match popNElements lst 4 with
          ...
          | _ => ffail end else if Checker.eq_term init_graph arg qIff then reifyIff tct qff lst fuel envTerm env (findPropRepresentation tct fuel) (fun tct => findTermRepresentation tct fuel) else ffail
    | (0,tProd x P Q) => if maybeD tct (P) then
    ...
    | (0,(tApp arg lst)) => if Checker.eq_term init_graph arg qI_P then match popNElements lst 4 with
          ...
          | _ => ffail end else if Checker.eq_term init_graph arg qIff then reifyIffNP tct qff lst fuel envTerm env (findPropRepresentationNP tct fuel) (fun tct => findTermRepresentationNP tct fuel) else ffail
    | (0,tProd x P Q) => if maybeD tct (P) then

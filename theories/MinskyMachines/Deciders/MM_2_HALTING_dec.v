...
Proof.
  by rw [LHS](vec_head_tail v) [in LHS](vec_head_tail (vec_tail v)) [in LHS](vec_0_nil (vec_tail (vec_tail v))).
Qed.
...
Proof.
  ...
  { rw /M HP map_app (decompose_vec2 v) nth_error_app2.
    { by rw /= length_map. }
    by rw /= length_map Nat.sub_diag. }
  move Hc: (1 + length P1, v) => c H.
  case: H Hc.
  - move=> ? d w [<- <-]. rw [to_Instruction _]/= (decompose_vec2 v).
    by case: (case_pos2 d) => /= ->.
  - move=> ? d q w + [<- ?]. subst w. rw (decompose_vec2 v) /=.
    case: (case_pos2 d).
    + move=> /= -> ->. by case: q.
    + move=> /= -> ->. by case: q.
  - move=> ? d q w u + [<- ?]. subst w. rw (decompose_vec2 v) /=.
    case: (case_pos2 d).
    + by move=> /= -> ->.
    + by move=> /= -> ->.
Qed.
...
Proof.
  ...
  - move=> c q.
    ...
    { move: p HP1 => [|p].
      { rw (decompose_vec2 v) /= HP length_app /=. lia. }
      move=> ? /=. lia. }
    move: HP1. rw (decompose_vec2 v) Hp /= => HP1.
    ...
    + exists (if (vec_head v) is S a then
      ...
      { case: (vec_head v).
        - by case: (r).
        - move=> ?. by rw [in to_Config _]Hp /to_Config. }
      case Hv: (vec_head v) => [|a].
      * exists 1, P1, (DEC pos0 r), P2, v.
        split; first by congr pair.
        split. 
        { by rw [in (_, v)](decompose_vec2 v) /= HP1 Hv. }
        have := @in_mm_sss_dec_0 2 p pos0 r v.
        rw [in vec_pos v](decompose_vec2 v) /=.
        move=> /(_ Hv).
        by congr mm_sss; rw -?Hp [in (_, v)](decompose_vec2 v) Hv.
      * exists 1, P1, (DEC pos0 r), P2, v.
        split; first by congr pair.
        split. 
        { by rw [in (_, v)](decompose_vec2 v) /= HP1 Hv. }
        ...
        move=> /(_ a Hv).
        congr mm_sss; rw -?Hp.
        ** by rw -Hv -(decompose_vec2 v).
        ** by rw [in vec_change v](decompose_vec2 v).
...
Proof.
  ...
  move=> n IH s /=. rw obind_oiter.
  case Hz: (MPM2.step M (to_Config s)) => [z|]; last by rw oiter_None.
...
Proof.
  ...
  - left. move: H => [n]. elim: n; first done.
    ...
    + move: Hy => /M_to_P [t] [->] Ht H't.
      ...
      rw (decompose_vec2 w) /to_Config /= /MPM2.step /=.
      case Hi: (nth_error M p) => [i|]; first last.
      { move: Hi => /nth_error_None. rw /M length_map. lia. }
      case: i Hi; [|done..].
  ...
  - right. move=> [t] [/=] /P_to_M [n] Hn Ht.
    ...
    rw /= /MPM2.steps Hn.
    move: t Ht {Hn} => [[|p] w].
    + rw /= /MPM2.step /to_Config (decompose_vec2 w) /=.
      suff ->: nth_error M (length P) = None by done.
      apply /nth_error_None. by rw /M length_map.
    + move=> /= [|?]; first by lia.
      rw /= /MPM2.step /to_Config (decompose_vec2 w) /=.
      suff ->: nth_error M p = None by done.
      apply /nth_error_None. rw /M length_map. lia.
Qed.
...
Proof.
  rw /decider /reflects /decide => - [P v].
  case: (decision P v).
  - tauto.

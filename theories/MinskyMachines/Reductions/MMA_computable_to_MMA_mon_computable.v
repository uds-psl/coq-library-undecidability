...
Proof.
  elim: v. { by rw vec_app_nil. }
  move=> > /= ->. by rw vec_app_cons.
Qed.
...
Proof.
  ...
  rw !Vector.splitat_append. by move=> [<- <-].
Qed.
...
Proof.
  rw /shift_counter [in RHS](eq_sym (pos_lr_both _ _ x)).
  case: (pos_both (1 + k) k' x) => [Y|Y].
  - have [->|[Y' ->]] := pos_S_inv Y.
    + by rw !vec_app_eq /= !vec_pos_app_right.
    + by rw !vec_app_eq /= !vec_pos_app_left.
  - by rw !vec_app_eq /= !vec_pos_app_right !vec_pos_app_left.
Qed.
...
Proof.
  rw /shift_counter [in (vec_change _ x _)](eq_sym (pos_lr_both _ _ x)).
  case: (pos_both (1 + k) k' x) => [Y|Y].
  - have [->|[Y' ->]] := pos_S_inv Y.
    + rw !vec_app_eq !vec_app_cons /= !vec_change_app_right /= -!vec_app_eq.
      by move=> /Vector.cons_inj [<-] /vec_append_inj [<- <-].
    + rw !vec_app_eq !vec_app_cons /= !vec_change_app_left -!vec_app_eq.
      by move=> /Vector.cons_inj [<-] /vec_append_inj [<- <-].
  - rw !vec_app_eq !vec_app_cons /= !vec_change_app_right !vec_change_app_left -!vec_app_eq.
    by move=> /Vector.cons_inj [<-] /vec_append_inj [-> ->].
Qed.
...
Proof.
  ...
  - move=> > ? /=.
    ...
    by rw vec_pos_shift_counter (vec_change_shift_counter E3).
  - move=> > ? i > ?.
    move=> /pair_equal_spec [??] /pair_equal_spec [?]. subst.
    move=> /Vector.cons_inj [?] /vec_append_inj [??]. subst.
    rw (shift_addr_range). { lia. }
    apply: in_mma_sss_dec_0'; [done..|].
    by rw vec_pos_shift_counter.
  - move=> > ? i > ?.
    move=> /pair_equal_spec [??] /pair_equal_spec [?] E3. subst.
    rw /=.
    apply: in_mma_sss_dec_1'; [done| |rw vec_pos_shift_counter; eassumption].
    by rw (vec_change_shift_counter E3).
Qed.
...
Proof.
  rw /P'. move=> [offset'] [l] [instr] [r] [v''].
  move=> [[<-]] /[dup] HP.
  move=> -> [/config_eqE2] [->] [->] [-> ->] Hinstr.
  eexists 1, (map shift_instr l), (shift_instr instr), (map shift_instr r ++ _), _.
  split; [|split].
  - congr pair. by rw map_app /= -app_assoc.
  - congr pair. rw length_map shift_addr_range //.
...
Proof.
  ...
  { eexists 1, _, _, _, _. split; [reflexivity|split].
    - rw length_map. reflexivity.
    - by constructor. }
  econstructor.
  { eexists 1, (map shift_instr P ++ [INC F1']), _, _, _.
    rw -app_assoc. split; [reflexivity|split].
    - rw length_app length_map /=. congr pair. lia.
    - apply: in_mma_sss_dec_1'; [done..|].
      by rw !vec_app_eq !vec_pos_app_right !vec_change_app_right !vec_pos_app_right. }
  rw -/(Nat.add _ _).
  rw !vec_app_eq !vec_pos_app_right !vec_change_app_right /=.
  elim: m n.
  { move=> n. rw Nat.add_0_r.
    econstructor; [|econstructor].
    eexists 1, (map shift_instr P ++ [_; _; _]), _, _, _.
    rw -app_assoc. split; [reflexivity|split].
    - rw length_app length_map /=. congr pair. lia.
    - apply: in_mma_sss_dec_0'; [done..|].
      by rw (vec_pos_app_right (n ## v)) vec_pos_app_right. }
  move=> m IH n.
  econstructor.
  { eexists 1, (map shift_instr P ++ [_; _; _]), _, _, _.
    rw -app_assoc. split; [reflexivity|split].
  ...
  have := IH (S n). rw -(Nat.add_succ_comm n m).
  congr sss_steps. congr pair.
  by rw vec_change_F1' -!vec_app_eq.
Qed.
...
Proof.
  ...
  move: t => [i' a'v'w'].
  rw (Vector.eta a'v'w') (vec_append_splitat (VectorDef.tl a'v'w')) Vector.splitat_append.
...
Proof.
  ...
  move=> [t'] [/syncE] /=. rw Vector.splitat_append /=.
  move=> [?] -> + /shift_addr_outcode Hc.
  rw Nat.sub_0_r Hc vec_append_const => H1.
...
Proof.
  ...
  - move=> v /(sss_terminates_iff (@mma_sss_total_ni _)) Hv. apply: H2P.
    ...
    rw -vec_append_const. by apply: sync_intro.
Qed.

...
Proof. 
  ...
  - by rw /map -/(map _ _) ?poly_add_nthP.
  - move=> i. rw /map -/(map _ _) /mset_to_poly -/(mset_to_poly).
    rw /= ?poly_add_nthP /=. by lia.
Qed.

Lemma poly_add_0I {p q r} : r ≃ [] -> p ≃ q -> p ≃ poly_add q r.
Proof.
  move=> + + i => /(_ i) + /(_ i). rw poly_add_nthP.
  case: i=> /=; by lia.
Qed.

Lemma poly_shiftI {p} : poly_mult [0; 1] p ≃ (0 :: p).
Proof.
  rw /poly_mult => i /=. rw poly_add_nthP /=.
...
Proof.
  move=> [φ]. rw -Forall_forall => Hφ.
  exists (fun x => mset_to_poly (φ x)). rw -Forall_forall Forall_map.
...
Proof.
  ...
  - move=> i IH. case; first done.
    move=> a p /=. rw count_occ_app.
    rw -(count_occ_map S Nat.eq_dec) ?IH; first by lia.
    by elim a.
Qed.

Lemma poly_to_mset_eqI {p q} : p ≃ q -> poly_to_mset p ≡ poly_to_mset q.
Proof. move=> + a. by rw ?count_occ_poly_to_msetP. Qed.

Lemma poly_to_mset_addP {p q} : poly_to_mset (poly_add p q) ≡ poly_to_mset p ++ poly_to_mset q.
Proof. move=> a. by rw count_occ_app ? count_occ_poly_to_msetP poly_add_nthP. Qed.
...
Proof.
  ...
  { move=> H1 H2 c. by rw (H1 c) (H2 c). }
  move=> [ψ]. rw -Forall_forall Forall_map => Hψ.
  exists (fun x => poly_to_mset (ψ x)). rw -Forall_forall.
  apply: Forall_impl; last by eassumption. case.
  ...
  - move=> x y /= /poly_to_mset_eqI. move /eq_trans. apply.
    move: (ψ y) => p. rw -poly_to_mset_consP. apply: poly_to_mset_eqI.

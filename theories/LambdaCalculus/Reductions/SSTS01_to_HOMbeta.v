...
Proof.
  ...
  rw /sym /symbols. elim: (rules); first done.
...
Proof. by rw /single !map_app /= !map_repeat. Qed.

Lemma length_single M N i : i < n -> length (single i M N) = n.
Proof. rw /single length_app /= !repeat_length. lia. Qed.

Lemma nth_single_eq i N1 N2 M : nth i (rev (single i N1 N2)) M = N1.
Proof.
  rw /single rev_app_distr /= -app_assoc.
  by rw app_nth2 !length_rev !repeat_length ?Nat.sub_diag.
Qed.

Lemma nth_single_neq i j N1 N2 M : i < n -> j < n -> i <> j -> nth j (rev (single i N1 N2)) M = N2.
Proof.
  move=> ???. rw /single rev_app_distr /= -app_assoc.
  have [?|?] : j < i \/ j > i by lia.
  - rw app_nth1; first by rw length_rev repeat_length.
    rw rev_repeat (nth_indep _ _ N2); first by rw repeat_length.
    by apply: nth_repeat.
  - rw app_nth2 !length_rev !repeat_length; first lia.
    have ->: j - i = S (j - i - 1) by lia.
    rw /= rev_repeat (nth_indep _ _ N2); first by (rw repeat_length; lia).
    by apply: nth_repeat.
Qed.

Lemma Forall_single P M N i : P M -> P N -> Forall P (single i M N).
Proof.
  move=> ??. rw /single. apply /Forall_app. split.
...
Proof.
  move=> ??. rw /single. apply: Forall2_app.
...
Proof.
  move=> ?. apply: stlc_var. rw nth_error_app1; first by rw repeat_length.
  by rw nth_error_repeat.
Qed.

Lemma stlc_pi Gamma i : i < n -> stlc Gamma (pi i) A.
Proof.
  rw /pi /A=> ?. apply: stlc_lams.
  - by apply: repeat_length.
  - rw rev_repeat. by apply: stlc_var_atom.
Qed.
...
Proof. move=> ?. by rw ren_closed; auto with allfv. Qed.

Lemma subst_pi i sigma : i < n -> subst sigma (pi i) = pi i.
Proof. move=> ?. by rw subst_closed; auto with allfv. Qed.
...
Proof.
  move=> ?. constructor.
  rw nth_error_app1; first by rw repeat_length.
  by apply nth_error_repeat.
Qed.

Lemma stlc_repeat_var_eq Gamma x t : stlc (repeat atom x ++ t :: Gamma) (var x) t.
Proof.
  apply: stlc_var. by rw nth_error_app2 repeat_length ?Nat.sub_diag.
Qed.

Lemma stlc_delta Gamma i : i < n -> stlc Gamma (delta i) (arr A A).
Proof.
  rw /delta=> ?. constructor.
  apply: stlc_lams; first by apply: repeat_length.
  rw rev_repeat. apply: stlc_apps_single; first done.
  - constructor. by rw nth_error_app2 repeat_length ?Nat.sub_diag.
...
Proof.
  elim /rev_ind: Ms x.
  - move=> /=. lia.
  - move=> M Ms IH. rw fold_left_app rev_app_distr /=.
    move=> [|?] /=; first done.
    rw length_app=> /= ?. apply: IH. lia.
Qed.
...
Proof.
  ...
  rw -length_rev -fold_left_rev_right.
  elim: (rev (single i N1 N2)) (n) x Hx.
  - move=> > ? /= <-. by rw Nat.sub_0_r.
  - move=> ?? IH [|?]; first done.
    move=> [|x] /=; first by lia.
...
Proof.
  move=> ?. rw /pi. apply: rt_trans; first by apply: steps_apps_lams_n_single.
  apply: steps_refl. by rw /= fold_left_single_eq.
Qed.

Lemma steps_pi_neq i j N1 N2 : i < n -> j < n -> i <> j -> steps (apps (pi i) (single j N1 N2)) N2.
Proof.
  move=> ???. rw /pi. apply: rt_trans; first by apply: steps_apps_lams_n_single.
  apply: steps_refl. by rw /= fold_left_single_neq.
Qed.
...
Proof.
  ...
  rw subst_lams subst_apps /= iter_up_ge; first done.
  rw Nat.sub_diag map_single /=.
  rw iter_up_lt; first done.
  rw iter_up_lt; first done.
  by apply: steps_refl.
Qed.
...
Proof.
  ...
  apply: steps_refl. rw subst_apps !map_single /=. congr (apps _ _).
  - rw fold_left_single_eq; first done.
    by rw fold_left_single_neq.
  - rw subst_ren_term /= -[RHS]subst_var_term. apply: ext_subst_term.
    move=> ?. rw fold_left_single_ge; [..|congr var]; lia.
Qed.

Lemma steps_delta_pi_top i : i < n -> steps (app (delta i) (pi top)) (pi i).
Proof.
  move=> ?. apply: rt_trans; first by apply: step_delta.
  apply: stepsLams. rw ren_pi; first done. by apply: steps_pi_eq.
Qed.
...
Proof. rw /pi. elim: (n)=> *; by do ? constructor. Qed.
...
Proof.
  ...
  rw /A /pi. elim /(Nat.measure_induction _ term_size): M HM (n).
  ...
  - move=> {}M /neutralE' [x] [Ms] [-> _] IH n k /stlc_appsE [ss] [HMs] /stlcE.
    move=> /[dup] /nth_error_Some'. rw arrs_arrs repeat_length.
    move=> /[dup] ? /(@nth_error_repeat ty) -> [] /(arrs_inj []).
    move: ss Ms n HMs IH=> [|??] [|??] [|?] /Forall2_length; [|done..].
    move=> *. rw Nat.add_0_r. by exists x.
Qed.
...
Proof. rw /case_of. by do 3 rw !subst_apps !map_single. Qed.
...
Proof.
  move=> H. rw /case_of. apply: stepsAppsR. apply: Forall2_single; first done.
  by apply: rt_refl.
Qed.
...
Proof. move=> *. rw /case_of. auto 8 with allfv. Qed.
...
Proof.
  move=> *. rw /case_of. apply: stlc_apps_single; [done..|].
...
Proof.
  move=> ?. apply: rt_trans.
  { apply: stepsReds'. by rw length_map length_rev length_seq. }
  apply: steps_refl. rw /= fold_left_lt.
  - by rw length_map length_rev length_seq.
  - by rw map_rev rev_involutive map_nth seq_nth.
Qed.
...
Proof. rw /Gamma_A !length_app !repeat_length /=. lia. Qed.

Lemma Gamma_A_pos m x : x < m -> nth_error (Gamma_A m) x = Some (arr A A).
Proof.
  move=> ?. rw /Gamma_A nth_error_app1 ?repeat_length; first done.
  by apply: nth_error_repeat.
Qed.

Lemma Gamma_A_space m : nth_error (Gamma_A m) m = Some A_star.
Proof.
  rw /Gamma_A nth_error_app2 ?repeat_length; first lia.
...
Proof.
  move=> ??. rw /Gamma_A nth_error_app2 ?repeat_length; first lia.
...
Proof.
  move=> ?. rw /sigma_F app_nth1 ?repeat_length; first done.
  rw (nth_indep _ _ (lam (var 0))) ?repeat_length; first done.
  by apply: nth_repeat.
Qed.

Lemma sigma_F_space m : sigma_F m m = F_star.
Proof.
  rw /sigma_F app_nth2 ?repeat_length; first lia.
  by have ->: m - m = 0 by lia.
Qed.
...
Proof.
  rw length_Gamma_A=> ??. rw /sigma_F app_nth2 ?repeat_length; first lia.
  rw (nth_indep _ _ F_R).
  { rw /= repeat_length. lia. }
  have ->: x - m = 3 + (x - m - 3) by lia.
  by apply: nth_repeat.
Qed.

Lemma stlc_F_0 Gamma : stlc Gamma F_0 A_0R.
Proof. rw /F_0 /A_0R. by do ? econstructor. Qed.

Lemma stlc_F_R Gamma : stlc Gamma F_R A_0R.
Proof. rw /F_R /A_0R. by do ? econstructor. Qed.

Lemma stlc_F_star Gamma : stlc Gamma F_star A_star.
Proof. rw /F_star /A_star. by do ? econstructor. Qed.
...
Proof.
  rw /sigma_F /= -(map_nth (ren S))=> H.
  rw map_app /= !map_repeat /=.
  apply: nth_indep.
  move: H. by rw /Gamma_A !length_app /= !repeat_length.
Qed.
...
Proof.
  apply: rt_trans.
  - rw /=. apply: stepsAppL. by apply: stepsRed.
  - by apply: stepsRed.
Qed.
...
Proof.
  move=> ?. rw /sigma_H app_nth1 ?repeat_length; first done.
  rw (nth_indep _ _ (delta bullet)) ?repeat_length; first done.
  by apply: nth_repeat.
Qed.

Lemma sigma_H_space m : sigma_H m m = H_star.
Proof.
  rw /sigma_H app_nth2 ?repeat_length; first lia.
  by have ->: m - m = 0 by lia.
Qed.
...
Proof.
  rw /H_star /=. do 2 constructor.
  apply: stlc_lams; first by rw repeat_length.
  rw rev_repeat. by auto with stlc.
Qed.

Lemma stlc_H_0 Gamma : stlc Gamma H_0 A_0R.
Proof.
  rw /H_0 /=. do 2 constructor. apply: stlc_lams; first by rw repeat_length.
  rw rev_repeat. by auto with stlc.
Qed.

Lemma stlc_H_R Gamma : stlc Gamma H_R A_0R.
Proof.
  rw /H_R /=. do 2 constructor. apply: stlc_lams; first by rw repeat_length.
  rw rev_repeat.
  have ? : forall t, stlc (repeat atom n ++ A :: t :: Gamma) (var (S n)) t.
  { constructor. rw nth_error_app2 repeat_length; first lia.
...
Proof.
  ...
  - move=> -> -> [<-]. rw sigma_H_space. by apply: stlc_H_star.
  - move=> -> -> [<-]. rw sigma_H_fin. by apply: stlc_H_1.
  - move=> -> -> [<-]. rw sigma_H_init. by apply: stlc_H_0.
  - move=> ? /[dup] /nth_error_Some' ? -> [<-].
    rw sigma_H_rule; [done..|].
    by apply: stlc_H_R.
Qed.
...
Proof.
  apply: rt_trans.
  { rw /H_0 /=. apply: stepsAppL. by apply: stepsRed. }
  apply: rt_trans.
  { rw /=. by apply: stepsRed. }
  apply: steps_refl.
  rw subst_subst_term!subst_lams !subst_apps !map_single /=.
  by rw iter_up_eq !iter_up_lt.
Qed.
...
Proof.
  apply: rt_trans.
  { rw /H_R /=. apply: stepsAppL. by apply: stepsRed. }
  apply: rt_trans.
  { rw /=. by apply: stepsRed. }
  rw !subst_lams !subst_apps !map_single /= !subst_apps !map_single /=.
  rw !subst_pi; [done..|].
  rw iter_up_eq !(iter_up_ge n (S n)); first lia.
  have -> : S n - n = 1 by lia.
  rw /= Nat.add_0_r iter_up_eq /=.
  apply: stepsLams. apply: steps_refl. congr (apps _ _).
  - rw !iter_up_lt; [done..|].
    by rw /= !iter_up_lt.
  - congr app. rw ren_ren_term subst_ren_term /= [RHS]ren_as_subst_term.
    apply: ext_subst_term=> x.
    rw iter_up_ge; first lia.
...
Proof.
  apply: rt_trans.
  { rw /H_star /=. apply: stepsAppL. by apply: stepsRed. }
  apply: rt_trans.
  { rw /=. by apply: stepsRed. }
  rw !subst_lams !subst_apps !map_single /= !subst_delta; [done..|].
  rw iter_up_eq !iter_up_lt; [done..|].
  apply: stepsLams. apply: steps_refl. congr (apps _ _).
  - by rw /= !iter_up_lt.
  - congr app. by rw /= Nat.add_0_r iter_up_eq.
Qed.

Lemma ren_S_sigma_H m x : x < length (Gamma_A m) -> ren S (sigma_H m x) = sigma_H (S m) (S x).
Proof.
  move=> Hx.
  rw ren_closed; first by auto with allfv.
  rw /sigma_H /=. apply: nth_indep.
  move: Hx. by rw /Gamma_A /= !length_app /= !repeat_length.
Qed.
...
Proof.
  move: k => [|k] ???; rw /G_pos.
  - rw (nth_indep _ _ (delta bullet)).
    + by rw repeat_length.
    + by rw nth_repeat.
  - set f := (fun _ => _).
    rw (nth_indep _ _ (f 0)); first by rw length_map length_seq.
    rw map_nth seq_nth; first done.
    rw /f /=.
    case E1: (Nat.eqb k x).
...
Proof.
  move=> ?. rw /sigma_G app_nth1; first by rw length_G_pos.
  rw /G_pos (nth_indep _ _ (delta bullet)).
  - by rw repeat_length.
  - by rw nth_repeat.
Qed.
...
Proof.
  move=> ?. rw /sigma_G app_nth1; first by rw length_G_pos.
  case E1: (Nat.eqb k x).
  { move=> /Nat.eqb_eq in E1. rw E1.
    rw /G_pos. set f := fun _ => _.
    rw (nth_indep _ _ (f 0)); first by rw length_map length_seq.
    rw map_nth seq_nth /=; first done.
    by rw /f Nat.eqb_refl. }
  move=> /Nat.eqb_neq in E1.
  case E2: (Nat.eqb k (S x)).
  { move=> /Nat.eqb_eq in E2. rw E2.
    rw /G_pos. set f := fun _ => _.
    rw (nth_indep _ _ (f 0)); first by rw length_map length_seq.
    rw map_nth seq_nth /=; first done.
    rw /f Nat.eqb_refl.
  ...
  by rw nth_G_pos_neq; [lia..|].
Qed.

Lemma sigma_G_fin m k : sigma_G m k (S m) = G_1.
Proof.
  rw /sigma_G app_nth2 length_G_pos; first lia.
  by have ->: S m - m = 1 by lia.
Qed.

Lemma sigma_G_star m k : sigma_G m k m = G_star.
Proof.
  rw /sigma_G app_nth2 length_G_pos; first done.
  by rw Nat.sub_diag.
Qed.

Lemma sigma_G_init m k : sigma_G m k (S (S m)) = G_0.
Proof.
  rw /sigma_G app_nth2 length_G_pos; first by lia.
  by have ->: S (S m) - m = 2 by lia.
Qed.
...
Proof.
  ...
  rw /=. elim: (rules) x; first by case.
  move=> ?? IH [|x] /=.
  - by move=> [->].
  - move=> /[dup] Hx /IH <-. apply: nth_indep.
    rw length_map. apply /nth_error_Some. by rw Hx.
Qed.
...
Proof.
  ...
  { move=> ?. apply: stlc_var. rw nth_error_app2 repeat_length; first lia.
    by have ->: S n - n = 1 by lia. }
...
Proof.
  ...
  { move=> ?. apply: stlc_var. rw nth_error_app2 repeat_length; first lia.
    by have ->: S n - n = 1 by lia. }
  rw rev_repeat. by apply: stlc_case_of; auto 6 with stlc.
Qed.
...
Proof.
  ...
  { move=> ?. apply: stlc_var. rw nth_error_app2 repeat_length; first lia.
    by have ->: S n - n = 1 by lia. }
  rw rev_repeat. apply: stlc_case_of; [by auto 6 with stlc| |by auto 6 with stlc..].
...
Proof.
  move=> Hx.
  have H'x : x < length (Gamma_A m) by (apply /nth_error_Some; rw Hx).
  move: (H'x)=> /Gamma_A_vals_spec []; rw Hx=> ? [->].
  - move: k=> [|k].
    + rw sigma_G_pos_0; first done.
      by apply: stlc_delta.
    + rw sigma_G_pos_S; first done.
      case: (Nat.eqb k x); first by apply: stlc_delta.
      case: (Nat.eqb k (S x)); by apply: stlc_delta.
  - subst x. rw sigma_G_star. by apply: stlc_G_star.
  - subst x. rw sigma_G_fin. by apply: stlc_G_1.
  - subst x. rw sigma_G_init. by apply: stlc_G_0.
  - rw length_Gamma_A in H'x.
...
Proof.
  rw /sigma_G. move=> ?.
  have [?|?] : x < m \/ x >= m by lia.
  - rw [LHS]app_nth1; first by rw length_G_pos; lia.
    rw [RHS]app_nth1; first by rw length_G_pos; lia.
    rw /=. apply: nth_indep.
    by rw repeat_length.
  - rw [LHS]app_nth2; first by rw length_G_pos; lia.
    rw [RHS]app_nth2; first by rw length_G_pos; lia.
    rw !length_G_pos.
    apply: nth_indep.
    rw /= length_map. lia.
Qed.
...
Proof.
  ...
  - move: k=> [|k] ?.
    + rw sigma_G_pos_0; first done.
      by rw sigma_G_pos_S; first lia.
    + by rw !sigma_G_pos_S; [lia..|].
  - rw /sigma_G. move=> ?.
    rw [LHS]app_nth2 length_G_pos; first by lia.
    rw [RHS]app_nth2 length_G_pos; first by lia.
    apply: nth_indep. rw /= length_map. lia.
Qed.
...
Proof.
  ...
  apply: rt_trans.
  { apply: stepsAppL. by apply: stepsRed. }
  rw subst_lam. apply: rt_trans; first by apply: stepsRed.
  apply: steps_refl.
  rw subst_subst_term subst_lams subst_case_of /=.
  do ? rw subst_apps map_single.
  rw /= !iter_up_eq !subst_delta; [done..|].
  rw (iter_up_ge n (S n)); first by lia.
  have ->: S n - n = 1 by lia.
  rw !iter_up_lt /=; [done..|].
  by rw subst_ren_term subst_var_term subst_pi.
Qed.
...
Proof.
  ...
  { apply: steps_case_of_head. apply: rt_trans.
    ...
    - rw ren_delta; first done. by apply: steps_delta_pi_top. }
  rw /=. apply: rt_trans.
  ...
  { move=> ??. rw subst_ren_term ren_subst_term /=. apply: ext_subst_term=> - [|?]; last done.
    by rw /= ren_delta. }
  apply: steps_refl. by rw !E.
Qed.
...
Proof.
  ...
  move=> /steps_lamsE. rw !ren_closed; [by auto with allfv..|].
...
Proof.
  ...
  { rw /G_0 /=. apply: stepsAppL. by apply: stepsRed. }
  apply: rt_trans.
  { rw /=. by apply: stepsRed. }
  rw subst_subst_term subst_lams subst_case_of.
  do 2 rw !subst_apps !map_single /=.
  rw (iter_up_ge _ (S n)); first by lia.
  have ->: S n - n = 1 by lia.
  rw iter_up_eq /(up _ 0) /(up _ 1) /= subst_pi; first done.
  rw !iter_up_lt; [done..|].
  apply: stepsLams. apply: rt_trans.
  { apply: steps_case_of_fst. apply: rt_trans.
    - apply: stepsAppsL. apply: steps_ren. by eassumption.
...
Proof.
  ...
  rw ren_subst_term subst_ren_term subst_delta; first done.
  apply: stepsLams. apply: rt_trans.
...
Proof.
  ...
  move=> /steps_lamsE. rw ren_pi; first done.
  apply: steps_var_elim.
...
Proof.
  move=> HN.
  apply: rt_trans.
  { rw /G_R /=. apply: stepsAppL. by apply: stepsRed. }
  apply: rt_trans.
  { rw /=. by apply: stepsRed. }
  rw !subst_lams. apply: stepsLams.
  apply: rt_trans.
  { rw !subst_case_of /= !subst_pi; [done..|].
    rw -Nat.iter_succ_r iter_up_eq.
    apply: steps_case_of_head. rw subst_ren_term. by apply: HN. }
  apply: rt_trans.
  { apply: steps_case_of_head. by apply: steps_delta_pi_top. }
  apply: rt_trans.
  { by apply: steps_pi_eq. }
...
Proof.
  ...
  { rw !subst_case_of /=.
    rw !subst_pi; [done..|].
    rw -/(up _) -Nat.iter_succ_r iter_up_eq /=.
    apply: steps_case_of_head. apply: rt_trans.
    - rw subst_ren_term. by apply: HN.
  ...
  rw !subst_apps /=.
  rw -Nat.iter_succ iter_up_lt; first lia.
  rw /= iter_up_eq /= !map_map map_single.
...
Proof.
  apply: steps_G_R_L'=> *. apply: steps_refl. by rw subst_delta.
Qed.
...
Proof.
  apply: steps_G_R_R'=> *. apply: steps_refl. by rw subst_delta.
Qed.
...
Proof.
  ...
  { apply: rt_trans.
    - apply: stepsAppsL. apply: steps_ren. by eassumption.
    - rw ren_pi; first done. by apply: steps_pi_full. }
  by apply: steps_var_absurd.
Qed.
...
Lemma steps_G_R_bullet_elim M i rule :
  ...
  steps M (pi i).
Proof.
  apply: steps_G_R_bullet_elim'=> ??. apply: steps_refl. by rw subst_delta.
Qed.
...
Proof.
  ...
  - abstract: H'. apply: HM=> *.
    ...
    { apply: rt_trans.
      - apply: stepsAppsL. apply: steps_ren. by eassumption.
      - rw ren_pi; first done. by apply: steps_pi_neq. }
    by apply: steps_var_absurd.
  - apply: steps_var_elim H'M.
    { apply: rt_trans.
      - apply: stepsAppsL. apply: steps_ren. by apply: H'.
      - rw ren_pi; first done. by apply: steps_pi_eq. }
...
Lemma steps_G_R_L_elim M rule a :
  ...
  fst (fst rule) = a /\ steps M (pi (sym (fst (snd rule)))).
Proof.
  apply: steps_G_R_L_elim'=> ??. apply: steps_refl. by rw subst_delta.
Qed.
...
Lemma steps_G_R_R_elim M rule b :
  ...
  snd (fst rule) = b /\ steps M (pi (sym (snd (snd rule)))).
Proof.
  apply: steps_G_R_R_elim'=> ??. apply: steps_refl. by rw subst_delta.
Qed.
...
Proof.
  ...
  { apply /nth_error_Some. by rw H''x. }
  move: (Hx) (H''x) => /Gamma_A_vals_spec [] H'x -> [].
  - rw sigma_H_pos; first done.
...
Proof.
  ...
  apply: unnest. { move: H'Ms => /Forall2_length. by rw length_map. }
  apply: unnest; first done.
  case.
  - (* x_fin *)
    move=> ?. subst x.
    move: (Hx). rw Gamma_A_fin=> - [/(arrs_inj [])] ?. subst ss.
    move: Ms H'Ms Hm IH {HMs}=> [|??] /Forall2_length; [|done].
    move=> *. by constructor.
  - (* x_rule _ _ *)
    move=> ?. have ? : x > S m by lia.
    have ? : x < m + 3 + length rules by (by rw -length_Gamma_A).
    move: Hx. rw Gamma_A_rule; [done..|].
    move=> - [/arrs_inj] ?. subst ss.
    ...
    rw !subst_apps /= sigma_F_rule; [done..|].
    apply: steps_var_elim; first by apply: steps_F_R.
    move=> Hm IH. rw sigma_H_rule; [done..|].
    apply: steps_pi_elim; first by apply: steps_H_R.
    rw !ren_closed; [apply: allfv_closed; apply: subst_sigma_H_closed; eassumption..|].
    ...
    + (* x_rule (lam _) _ *)
      ...
      * move=> /= Hy Hm IH /steps_lamsE.
        apply: steps_var_elim; first by apply /stepsAppsL /stepsRed.
        have ? : y < length (Gamma_A m) by (apply /nth_error_Some; rw Hy).
        rw subst_subst_term subst_apps /= subst_ren_term.
        rw (subst_closed _ (sigma_H m y)); first by auto with allfv.
        ...
        { apply: stlc_apps.
          ...
          - apply /Forall2_map_l. apply: Forall2_impl H'Ms.
            ...
            + move=> ? /[dup] ? /nth_error_Some' Hz. rw subst_ren_term subst_closed.
              * by auto with allfv.
              * by apply: stlc_sigma_H. }
        apply: unnest; first done.
        apply: unnest; first done.
        move=> ?. move: Hy. rw Gamma_A_pos; first done.
        move=> [] /(arrs_inj [_]) ?. subst ss.
        move: Ms H'Ms HMs Hm IH=> [|{}N1 [|??]] /[dup] /Forall2_length; [done| |done].
        move=> _ /Forall2_cons_iff [H'N1] _ /Forall_cons_iff [HN1] _.
        rw !subst_apps subst_ren_term sigma_F_pos /=; first done.
        apply: steps_var_elim; first by apply: stepsRed.
        case /normal_form_nf: HN1 H'N1; first by move=> ??? /steps_lamE.
        ...
        { (* ring2 intro eta *)
          ...
          constructor; [|lia..].
          apply: IH; [lia|done..|].
          move: H_lift. rw sigma_H_pos; first done.
        ...
        move=> /= Hz Hm _. rw sigma_H_pos; first done.
        apply: steps_var_elim; first by apply: steps_delta_eq.
        rw subst_apps /=.
        have ? : z < length (Gamma_A m) by (apply /nth_error_Some; rw Hz).
        rw subst_ren_term (subst_closed _ (sigma_H m z)); first by auto with allfv.
    ...
    + move=> {}N1 /neutralE' [y] [Ms] [->] _ /stlc_appsE [ss] [H'Ms] /stlcE Hy Hm IH.
      rw subst_apps /= (apps_apps _ [_]) apps_apps app_assoc -apps_apps.
      move=> /[dup]. move: (Hy). rw (arrs_arrs _ [_]).
      move=> + /steps_lamsE.
      have ? : y < length (Gamma_A m) by (apply /nth_error_Some; rw Hy).
      move=> /well_typed_neutral_shape' /[apply].
      apply: unnest. { rw !length_app !length_map. by move: H'Ms => /Forall2_length ->. }
      apply: unnest.
      { apply: stlc_apps.
      - apply: stlc_sigma_H.
        move: Hy. change (arrs ss (arr A A)) with (arrs ss (arrs [A] A)).
        rw arrs_arrs. by apply.
      - apply: Forall2_app.
        + apply /Forall2_map_l. apply: Forall2_impl H'Ms.
      ...
      move=> _ /=. rw sigma_F_pos; first done.
      apply: steps_var_elim; first by apply: stepsRed.
      move=> /= Hm IH H_lift.
      have ->: x = (x - 3 - m) + 3 + m by lia.
      constructor; [|lia..].
      apply: IH; [lia|done..|].
      move: H_lift=> /steps_lamsE. rw sigma_H_pos; first done.
      apply: steps_var_elim; first by apply: steps_delta_eq.
      apply: steps_var_elim; first by apply: steps_pi_eq.
      by apply: steps_subst_sigma_H_pi_intro.
Qed.
...
Proof.
  ...
  apply: unnest. { rw length_map. by apply: Forall2_length H'Ms. }
  apply: unnest; first done.
  case=> ?; subst x.
  - (* x_space _ *)
    move: Hx. rw Gamma_A_space=> - [/(arrs_inj [_; _])] ?. subst ss.
    move: Ms H'Ms HMs Hm IH (H'Ms)=> [|? [|{}M [|??]]] /Forall2_length; [done|done| |done].
    move=> _ /Forall_cons_iff [_] /Forall_cons_iff [HM] _.
    rw /= sigma_F_space sigma_H_space.
    ...
    + (* x_space (lam _) *)
      ...
      move=> {}M HM. rw subst_subst_term /= => Hm IH.
      ...
      * move: Hm => /(steps_ren S). congr steps.
        ...
        move=> /nth_error_Some ?. rw subst_ren_term /= subst_var_term.
      ...
      * move: H_lift. apply: steps_pi_elim.
        ...
        { rw subst_ren_term subst_subst_term.
          apply: ext_stlc_subst_term H'M=> - [|x] /=; first done.
          move=> /nth_error_Some ?.
          rw subst_ren_term subst_closed; first by auto with allfv.
          rw -ren_S_sigma_H; first done.
          by rw ren_closed; auto with allfv. }
        by apply: steps_subst_sigma_H_pi_intro.
    + (* x_space (apps (var y) _) - typing error / semantic error *)
      ...
      have : y < length (Gamma_A m) by apply /nth_error_Some; rw Hy.
      rw (arrs_arrs _ [_]) in Hy.
      move=> /Gamma_A_vals_spec [] ?; rw Hy.
      * move=> [] /(arrs_inj _ [_]). by move: (ss) => [|? [|??]].
      * move=> [] /arrs_inj. by move: (ss) => [|?[|?[|??]]].
      * move=> [] /= /(f_equal ty_size). rw ty_size_arrs map_app list_sum_app /=. lia.
      * move=> [] /arrs_inj. by move: (ss) => [|?[|?[|??]]].
      * move=> [] /arrs_inj. by move: (ss) => [|?[|?[|??]]].
  - (* x_0 _ _ *)
    move: Hx. rw Gamma_A_init.
    move=> [/arrs_inj] ?. subst ss.
    move: Ms H'Ms HMs Hm {IH} (H'Ms)=> [|M1 [|M2 [|??]]] /Forall2_length; [done|done| |done].
    move=> _ /Forall_cons_iff [HM1] /Forall_cons_iff [HM2] _.
    rw !subst_apps /= sigma_H_init sigma_F_init.
    ...
    move=> /steps_lamsE. rw ren_closed.
...
Proof.
  ...
  - move=> k1 M /normal_form_nf [].
    + move=> ? /normal_form_nf []; first by eexists.
      ...
      * move=> x /= /[dup] H. rw nth_error_repeat.
        { rw -(repeat_length A_0R k1). apply /nth_error_Some. by rw H. }
        move=> [] /(f_equal ty_size). rw ty_size_arrs /=. lia.
    + move=> ? /neutralE' [x] [?] [->] _ /stlc_appsE [ss] [_] /stlcE.
      move=> /[dup] H. rw nth_error_repeat.
      { rw -(repeat_length A_0R k1). apply /nth_error_Some. by rw H. }
      move=> [] /(f_equal ty_size). rw ty_size_arrs /=. lia.
...
Proof.
  ...
  rw app_assoc -arrs_arrs.
  have E: (length rules + 1 + 2) = length (repeat A_0R (length rules + 1) ++ [A; A_star]) by rw length_app repeat_length.
  rw [in stlc _ _ _]E.
  move=> /stlc_lamsE. rw repeat_app !rev_app_distr /= rev_repeat app_nil_r.
  case /normal_form_nf: HN.
  - move=> {}N HN /stlcE [] ?? [<- <-] + HFN HHN.
    move: HN => /(construct_ring1 _ 1) + /[dup] H'N => /[apply].
    apply: unnest.
    { apply: steps_var_elim HFN.
      { rw (lams_lams _ 1). apply: stepsReds'.
        by rw length_app repeat_length /=; lia. }
      move=> /(steps_ren S). congr steps.
      rw ren_subst_term /sigma_F !fold_left_app.
      apply: ext_stlc_subst_term H'N=> - [|[|[|[|x]]]] /=; [done..|].
      move=> /nth_error_Some. rw repeat_length=> H.
      rw fold_left_lt; first by (rw repeat_length; lia).
      rw rev_repeat -(map_nth (ren S)) map_repeat ren_closed; first by auto with allfv.
      apply: nth_indep. rw repeat_length. lia. }
    apply: unnest.
    { move: HHN. rw (lams_lams _ 1). apply: steps_pi_elim.
      { apply: stepsReds'. rw length_app repeat_length /=. lia. }
      congr steps. rw /sigma_H !fold_left_app.
      apply: ext_stlc_subst_term H'N=> - [|[|[|[|x]]]] /=; [done..|].
      move=> /nth_error_Some. rw repeat_length=> H.
      rw fold_left_lt; first by (rw repeat_length; lia).
      rw rev_repeat. apply: nth_indep. rw repeat_length. lia. }
    move=> ?. exists N. rw (lams_lams _ 1). split; last done.
    congr (lams _ _). lia.
  - (* typing contradiction *)
    move=> ? /neutralE' [x] [Ms] [->] HMs /stlc_appsE [ss] [H'Ms] /stlcE.
    move: x=> [|[|x]] /=.
    + rw (arrs_arrs _ [_]). move=> [] /arrs_inj.
      by move: (ss)=> [|?[|?[|??]]].
    + move=> [] /(f_equal ty_size). rw ty_size_arrs /=. lia.
    + move=> /[dup] Hx.
      have {}Hx: x < 1 + length rules.
      { have /nth_error_Some: nth_error (repeat A_0R (1 + length rules)) x <> None by rw Hx.
        by rw repeat_length. }
      change (_ :: repeat A_0R ?k) with (repeat A_0R (S k)).
      rw nth_error_repeat; first done.
      rw (arrs_arrs _ [_]). move=> [] /arrs_inj.
      by move: (ss)=> [|?[|?[|??]]].
Qed.
...
Proof.
  move=> Hi H0M.
  apply: rt_trans.
  { apply: stepsReds'. by rw length_app length_rev length_map /=. }
  rw fold_left_app. apply: steps_refl.
  ...
  { move=> [|[|[|[|x]]]] k; [done..|].
    ...
    { rw length_Gamma_A in Hx. lia. }
    rw fold_left_lt ?rev_involutive; first by rw length_rev length_map.
    apply: nth_error_nth. rw nth_error_map.
    move: (H'x) => /nth_error_Some.
    case E: (nth_error rules x) => [rule|]; last done.
    have ->: S (S (S (S x))) = x + 3 + 1 by lia.
    by rw (sigma_G_rule _ _ x rule E). }
...
Proof. 
  ...
  - move=> {}m {}N N' ? IH.
    ...
    move=> /stlcE. rw Gamma_A_space=> - [??]. subst.
    move: (HN) => /stlcE [] ?? [<- <-] H'N.
    rw !subst_apps /= !sigma_G_star.
    ...
    { rw subst_subst_term.
      ...
      move=> /nth_error_Some.
      rw subst_ren_term /= subst_var_term length_Gamma_A.
    ...
    { rw subst_subst_term. apply: ext_stlc_subst_term H'N=> - [|?] /=; first done.
      move=> /nth_error_Some. rw length_Gamma_A=> ?.
      by rw subst_ren_term subst_var_term sigma_G_S_shift. }
    move=> {}H''N H.
    move: IH. apply: unnest.
    { rw /= -seq_shift. constructor; first done.
      apply /Forall_map. apply: Forall_impl H.
      move=> i. rw sigma_G_star.
      ...
      { change (lam (subst _ N)) with (subst (sigma_G m (S (S i))) (lam N)).
        apply: stlc_subst; first by eassumption.
        by apply: stlc_sigma_G. }
      congr steps. rw subst_subst_term.
      apply: ext_stlc_subst_term H'N=> - [|?] /=; first done.
      move=> /nth_error_Some. rw length_Gamma_A=> ?.
      by rw subst_ren_term subst_var_term sigma_G_S_shift. }
    move=> [M] [k] [?] [?] [?] [?] ?.
  ...
  - move=> {}m {}N M HN /stlcE [] ? /stlcE [] ? /stlcE.
    ...
    apply: Forall_impl H => - [|y] /=; rw sigma_G_init.
...
Proof.
  ...
  - move=> x y {}M ? IH Hx Hy v Hm Hv.
    ...
    { have /(nth_split _ 0) : y < length v by lia.
      move=> [u1] [[|b u2]].
      - move=> [/(f_equal (@length nat))]. rw length_app /=. lia.
    ...
    { move: H'v => /(f_equal (fun (l : list nat) => nth_error l (length l1))).
      rw nth_error_seq; first lia.
      rw nth_error_app2; first done.
      rw Nat.sub_diag /=.
      by move=> [<-]. }
    have ? : i2 = S i1.
    { move: H'v => /(f_equal (fun (l : list nat) => nth_error l (S (length l1)))).
      rw nth_error_seq; first lia.
      rw nth_error_app2; first lia.
      have ->: S (length l1) - length l1 = 1 by lia.
      move=> [<-]. lia. }
    subst.
    (* left symbol transition *)
    move: (E) Ha => /sigma_G_rule ->.
    rw sigma_G_pos_S; first lia.
    rw Nat.eqb_refl=> /steps_G_R_L_elim.
    ...
    apply: unnest. { move: Hv. by rw !length_app. }
    apply: unnest.
  ...
  - move=> x y {}M ? IH Hx Hy v Hm Hv.
    ...
    move: (E)=> /sigma_G_rule ->.
    rw sigma_G_pos_0; first done.
    rw ren_delta; first done.
    move=> /steps_G_R_bullet_elim'.
    apply: unnest.
    { move=> *. apply: rt_trans.
      - move=> /=. by apply: stepsRed.
      - apply: steps_refl. by rw /= subst_subst_term subst_delta. }
    apply: unnest; first done.
    apply: unnest. { by apply: stlc_subst_sigma_G. }
    move=> /IH {}IH H.
    have [u1 [a [b [u2 [??]]]]] : exists u1 a b u2, v = u1 ++ a :: b :: u2 /\ length u1 = y.
    ...
    move: (E) Ha => /sigma_G_rule ->.
    rw sigma_G_pos_S; first lia.
    rw Nat.eqb_refl ren_delta; first done.
...
Proof.
  ...
  { move: H2N => /stlc_lamsE'.
    rw length_app repeat_length -Nat.add_assoc /=.
    rw rev_app_distr rev_repeat /= app_nil_r (Nat.add_comm _ 1).
    by apply. }
  ...
  apply: (sigma_G_ring_2_elim _ (m + 1) M'); [lia|by rw repeat_length|done..|].
  rw /seq /= -/seq. constructor; first done.
  apply: Forall2_repeat_r'; last by rw length_seq.
  rw -seq_shift. apply /Forall_map.
...
Proof.
  ...
  { rw ren_subst_term subst_subst_term.
    apply: ext_stlc_subst_term HM=> - [|?] /=; first done.
    move=> /nth_error_Some ?.
    by rw subst_ren_term /= -ren_as_subst_term -ren_S_sigma_F. }
  move=> /(steps_ren Nat.pred). rw ren_ren_term ren_id_term /=.
  ...
  - move=> m IH M HM H'M. apply: IH.
    { apply: stlc_apps.
      ...
      - constructor; [|constructor; [|constructor]].
        + by apply: stlc_var.
        + by apply: stlc_lam. }
    rw subst_apps /= subst_ren_term subst_var_term sigma_F_space.
...
Proof.
  ...
  apply: rt_trans.
  { apply: stepsReds'. by rw length_app repeat_length /=. }
  ...
  { move: (H') => /stlc_init_tape /steps_sigma_F_expand_tape. apply.
    ...
    - move=> > ? IH ?? /stlcE [] ? /stlcE [] ? /stlcE + _.
      rw Gamma_A_rule; [lia..|].
      move=> [_ <-] ?.
      rw /= sigma_F_rule; [rw ?length_Gamma_A; lia..|].
      rw sigma_F_pos; first done.
    ...
    - move=> > ? IH ?? /stlcE [] ? /stlcE [] ? /stlcE + _.
      ...
      { rw /=. by apply: stepsRed. }
      by apply: IH.
    - move=> _. rw /= sigma_F_fin. by apply: rt_refl. }
  apply: steps_refl.
  rw /sigma_F. rw fold_left_app subst_subst_term.
  apply: ext_stlc_subst_term H=> - [|[|[|[|x]]]] /= Hx; [done..|].
  have ? : x < length (repeat F_R (length rules)).
  { move: Hx=> /nth_error_Some. by rw !repeat_length. }
  rw fold_left_lt; first done.
  rw rev_repeat subst_ren_term /= subst_var_term.
  by apply: nth_indep.
Qed.
...
Proof.
  ...
  { apply: stlc_apps.
    ...
    - constructor; [|constructor; [|constructor]].
      + by apply: stlc_var.
      + by apply: stlc_lam. }
  rw subst_apps /= sigma_H_space sigma_H_pos; first by lia.
  apply: rt_trans; first by apply: steps_H_star.
  apply: stepsLams.
  apply: rt_trans.
  { rw /=. apply: stepsAppsL. by apply: stepsRed. }
  rw subst_ren_term subst_subst_term/=.
  apply: rt_trans.
  { apply: stepsAppsL. move: H'M. congr steps.
    apply: ext_stlc_subst_term HM=> - [|?] /=; first done.
    move=> /nth_error_Some /= ?. rw ren_S_sigma_H; first done.
    rw subst_closed; last done.
    apply: allfv_closed. apply: sigma_H_closed. rw /=. lia. }
  by apply: steps_pi_eq.
Qed.
...
Proof.
  move=> H. rw /init_tape /= sigma_H_init.
...
Proof.
  ...
  { move: (H') => /stlc_init_tape /steps_sigma_H_expand_tape. apply.
    ...
    - move=> > ? IH ?? /stlcE [] ? /stlcE [] ? /stlcE + _.
      rw Gamma_A_rule; [lia..|].
      move=> [_ <-] H.
      rw /= sigma_H_rule; [rw ?length_Gamma_A; lia..|].
      rw sigma_H_pos; first done.
      apply: rt_trans; first by apply: steps_H_R.
      apply: stepsLams. apply: rt_trans.
      { rw /= ren_delta; first done. by apply: steps_delta_eq. }
      apply: rt_trans; first by apply: steps_pi_eq.
      rw ren_closed.
    ...
    - move=> > ? IH ?? /stlcE [] ? /stlcE [] ? /stlcE + _.
      ...
      rw /= sigma_H_rule; [rw ?length_Gamma_A; lia..|].
      apply: rt_trans; first by apply: steps_H_R.
      apply: stepsLams. rw /= sigma_H_pos; first done.
      apply: rt_trans.
      { apply: stepsAppsL. by apply: stepsRed. }
      apply: rt_trans.
      { rw /= !subst_ren_term subst_delta; first done. by apply: steps_delta_eq. }
      apply: rt_trans; first by apply: steps_pi_eq.
      rw ren_closed.
      { apply: allfv_closed. apply: subst_sigma_H_closed. by eassumption. }
      apply: rt_trans.
      { apply: stepsAppsL. by apply: IH. }
      by apply: steps_pi_eq.
    - move=> _. rw /= sigma_H_fin. by apply: rt_refl. }
  apply: steps_refl. rw fold_left_app /sigma_H.
  apply: ext_stlc_subst_term H=> - [|[|[|[|x]]]] /= Hx; [done..|].
  have ? : x < length (repeat H_R (length rules)).
  { move: Hx=> /nth_error_Some. by rw !repeat_length. }
  rw fold_left_lt; first done.
  rw rev_repeat. by apply: nth_indep.
Qed.
...
Proof.
  ...
  { move=> ? <-. exists (var (S (S m))). split; [|split; [|split]].
    - by constructor.
    - constructor. by rw Gamma_A_fin.
    - rw /= sigma_G_fin. by apply: rt_refl.
    - apply: Forall2_repeat_r'; last by rw length_seq.
  ...
  have ? : length u1 < S m.
  { move: H3M => /Forall2_length. rw length_app /= length_seq. lia. }
  have ? : x < length rules by (apply /nth_error_Some; rw Hx).
  split; [|split; [|split]].
  - by constructor.
  - apply: stlc_app; [apply: stlc_app; apply: stlc_var|].
    + rw Gamma_A_rule; by [|lia].
    + by rw Gamma_A_pos.
    + done.
  - rw /=. move: (Hx)=> /sigma_G_rule ->.
    rw sigma_G_pos_0; first done.
...
Proof.
  ...
  { move=> [N] [HN] [H0N] [H1N] [H2N H3N]. exists N. split; [|split]; [done..|].
    ...
    { rw /=. split; first lia. move: HN => /ring2_allfv.
    ...
    - rw /= sigma_G_star (sigma_G_pos_0 _ 0); first lia.
      apply: steps_G_star_dollar.
      + by apply: rt_refl.
      + move: H1N. congr steps.
        rw subst_subst_term. apply: ext_allfv_subst_term. apply: allfv_impl HN.
        move=> [|x] /= ?; first done.
        rw subst_ren_term /= subst_var_term.
    ...
    - rw /= sigma_G_star (sigma_G_pos_S _ 0) /=; first lia.
    ...
    - apply /Forall_forall=> - [|x].
      + move=> _. rw /= sigma_G_star sigma_G_pos_S /=; first lia.
        ...
        * move: H3N => /= /Forall_cons_iff [_] /Forall_cons_iff [+ _]. congr steps.
          ...
          apply: sigma_G_S_shift. lia.
      + move=> /in_seq ?.
        rw /= sigma_G_star sigma_G_pos_S /=; first lia.
  ...
  { move=> [N] [HN] [H0N] [H1N H2N]. exists N. split; [|split]; [done..|].
    rw /init_tape /=. split; [|split].
    - rw sigma_G_init sigma_G_pos_0; first lia.
      ...
      { apply: stepsAppsL. apply: steps_ren. by eassumption. }
      rw ren_pi; first done. by apply: steps_pi_eq.
    - rw sigma_G_init sigma_G_pos_S /=; first lia.
    ...
    - move: H2N => /= /Forall_cons_iff [_].
      have ->: 1 :: seq 2 m = map S (seq 0 (S m)) by rw /= seq_shift.
      move=> /Forall_map.
  ...
  move: H_ssts => /ssts_simulate_rw [N] [?] [? [? /Forall2_repeat_r HN]].
  exists N. split; [|split; [|split]]; [done..|].
  apply: Forall_impl HN. by move=> ? [].
Qed.

(* if ssrs rws zeroes to ones, then there is a solution of the higher-order matching problem *)
...
Proof.
  ...
  split; [|split; [|split]].
  - apply: stlc_lams.
    + rw length_app repeat_length /=. lia.
    + move: H'0M. congr stlc.
      by rw /Gamma_A app_nil_r rev_app_distr rev_repeat (Nat.add_comm 1).
...
Proof.
  ...
  - apply: stlc_lam. apply: stlc_apps; first by apply: stlc_var.
    ...
    + by auto 7 with stlc.
  - apply: stlc_apps; first by apply: stlc_var.
    rw repeat_app -app_assoc /=. apply: Forall2_app.
    + apply: Forall2_repeat. by apply: stlc_H_R.
    + by auto 6 with stlc.
  - apply: stlc_apps; first by apply: stlc_var.
    rw repeat_app -app_assoc /=. apply: Forall2_app.
    + apply: Forall2_repeat_r'; last by rw length_rev length_map.
...
Proof.
  rw /HOM_b /HOM_B. apply: stlc_lam. apply: stlc_apps.
  - by apply: stlc_var.
...
Proof.
  ...
  { move=> xi. move: HM => /stlc_closed ?.
    rw ren_closed; last done. by apply: allfv_closed. }
  rw /HOM_f. apply: rt_trans; first by apply: stepsRed.
  rw subst_lam subst_apps. apply: stepsLam. apply: steps_refl.
  congr (apps _ _). rw /= !subst_apps /= !ren_M.
  move=> [: H]. congr ([_; _; _; _; _]).
  - by rw map_app map_repeat.
  - rw map_app map_repeat /= !subst_closed; auto with allfv.
  - rw map_app. congr (apps _ _). congr (_ ++ _).
    + abstract: H.
      rw map_rev map_map. congr rev. apply: map_ext_Forall.
      apply: Forall_impl (wf_rules)=> ??.
      rw subst_closed; auto with allfv.
    + by rw /= !subst_closed; auto with allfv.
  - rw map_app H. by rw /= !subst_closed; auto with allfv.
  - rw map_app H. by rw /= !subst_closed; auto with allfv.
Qed.
...
Proof.
  ...
  - move=> /correctness [M] [H1M] [H2M] [H3M] [H4M] [H5M] H6M.
    ...
    rw /HOM_b. apply: stepsLam. apply: stepsAppsR.
  ...
  - move=> [M] [H1M H2M]. apply /correctness.
    ...
    + rw /HOM_b. move=> /steps_lamE /steps_apps_varE.

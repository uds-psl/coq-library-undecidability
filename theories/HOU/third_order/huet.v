...
From Stdlib Require Import RelationClasses Morphisms List Arith Lia Init.Nat Setoid.
...
Section HuetReduction.
  ...
  Section ForwardDirection.
    ...
    Proof.
      ...
      rewrite AppR_Lambda'; simplify; (auto 2).
      unfold ginst; cbn [ren]; rewrite stepBeta.
      2: asimpl; simplify; cbn; unfold funcomp; (auto 2).
      cbn [ren]. rewrite it_up_ren_ge; simplify; (auto 2).
      asimpl. rewrite select_variables_subst; simplify; (auto 2).
      rewrite select_repeated; (auto 2).
      unfold ginst; cbn; asimpl; simplify.
      rewrite sapp_ge_in; simplify; (auto 3).
    ...
    Proof.
      ...
      - cbn -[ginst].  do 3 eapply equiv_lam_proper.
        ...
        all: rewrite !AppR_subst; rewrite ?Enc_subst_id; (auto 2).        
        ...
        rewrite !finst_equivalence, H1; simplify; (auto 2). 
        rewrite ginst_equivalence; trivial. 
  ...
  Section BackwardDirection.
    ...
    Proof using x t sigma N H1 EQ.
      ...
      all: destruct (sigma h') eqn: H'; cbn in *; intuition idtac.
      1, 3: eapply nth_error_In in H as H7; eapply H2 in H7.
      1, 2: eapply nth_error_sapp in H; rewrite ?H in EQ.
      + destruct s'; cbn in *; intuition idtac.  Discriminate.
      + rewrite AppR_subst in EQ; cbn in EQ. 
        eapply equiv_app_elim in EQ as (EQ1 & EQ2); cbn; trivial; simplify.
        destruct s'; cbn in *; intuition idtac.
        rewrite H in EQ1. 2: rewrite H; (auto 3).
        exfalso. symmetry in EQ1.
        eapply equiv_neq_var_app; (eauto 1); simplify; (eauto 3).  
      + exists h'. exists t1. intuition idtac. eauto using nth_error_Some_lt.
      Unshelve. exact 0.
    Qed.
  ...
  Proof.
    ...
    eapply equiv_app_elim in H as [EQ2 EQ1]; cbn; intuition idtac. 
    eapply equiv_app_elim in EQ2 as [_ EQ2]; cbn; intuition idtac.  
    ...
    assert (|L | <= |S'|) as L1 by
          (eapply (f_equal arity) in H1; simplify in H1; rewrite H1; trivial).
    symmetry in H1; eapply Arr_inversion in H1 as (L2 & H1 & H2); simplify; trivial.
    eapply repeated_app_inv in H1 as (H1 & H3 & H4); trivial.
    ...
    edestruct (@list_decompose_alt  k _ S') as (S1 & S2 & H4 & H5); [lia|]; subst S'. 
    ...
    rewrite !AppR_Lambda' in EQ1, EQ2; simplify; trivial.
